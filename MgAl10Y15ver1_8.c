...
int main(void) {
	...
	double
		...
		Zn_b[N] = { 2.99460, 0.203100, 7.08260, 18.0995 },
		 ...
		 Y_b[N] = { 1.35417, 11.2145, 22.6599, -0.01319 };
	...
	for (h =0; h <= 30; h++) {
		for (k =0; k <= h; k++) {
			for (l = 0; l <= 30; l++) {
				...
				x = ((Ka / 2.0) *sqrtl((4.0 / 3.0 * ((powl(h, 2.0) + h*k + powl(k, 2.0)) / powl(a, 2.0)) + powl(l, 2.0) / powl(c, 2.0))));

				if (x <= 1 && x >= -1) {

					theta = asin(x);
					degTheta = 2.0 * theta * 180.0 / M_PI;
					L = (1.0 + powl(cos(2.0 * theta), 2.0)) / (2.0 * powl(sin(theta), 2.0) * cos(theta));
					S = powl((sin(theta) / Ka), 2.0);
					...
					***–³Œø‰»***
					long d;
					for (d = 0; d < N; d++) {
					fs_Mg += ((Mg_a[d]) / expl(Mg_b[d] * S));
					fs_Al += ((Al_a[d]) / expl(Al_b[d] * S));
					fs_Zn += ((Zn_a[d]) / expl(Zn_b[d] * S));
					fs_Y  += (( Y_a[d]) / expl( Y_b[d] * S));
					}

					*/

					fs_Mg = ((Mg_a[0]) / (expl(Mg_b[0] * S))) + ((Mg_a[1]) / (expl(Mg_b[1] * S))) + ((Mg_a[2]) / (expl(Mg_b[2] * S))) + ((Mg_a[3]) / (expl(Mg_b[3] * S))) + C_Mg;
					fs_Al = ((Al_a[0]) / (expl(Al_b[0] * S))) + ((Al_a[1]) / (expl(Al_b[1] * S))) + ((Al_a[2]) / (expl(Al_b[2] * S))) + ((Al_a[3]) / (expl(Al_b[3] * S))) + C_Al;
					fs_Zn = ((Zn_a[0]) / (expl(Zn_b[0] * S))) + ((Zn_a[1]) / (expl(Zn_b[1] * S))) + ((Zn_a[2]) / (expl(Zn_b[2] * S))) + ((Zn_a[3]) / (expl(Zn_b[3] * S))) + C_Zn;
					fs_Y  = (( Y_a[0]) / (expl( Y_b[0] * S))) + (( Y_a[1]) / (expl( Y_b[1] * S))) + (( Y_a[2]) / (expl( Y_b[2] * S))) + (( Y_a[3]) / (expl( Y_b[3] * S))) + C_Y ;

					f_Mg = sqrtl(powl((fs_Mg + f1_Mg), 2) + powl(f2_Mg, 2));
					f_Al = sqrtl(powl((fs_Al + f1_Al), 2) + powl(f2_Al, 2));
					f_Zn = sqrtl(powl((fs_Zn + f1_Zn), 2) + powl(f2_Zn, 2));
					f_Y  = sqrtl(powl(( fs_Y +  f1_Y), 2) + powl( f2_Y, 2));
					...
					F_Mg = powl((f_Mg * Re_Mg), 2) + powl((f_Mg * Im_Mg), 2);
					F_Al = powl((f_Al * Re_Al), 2) + powl((f_Al * Im_Al), 2);
					F_Zn = powl((f_Zn * Re_Zn), 2) + powl((f_Zn * Im_Zn), 2);
					F_Y = powl((f_Y  * Re_Y), 2) + powl((f_Y  * Im_Y), 2);
